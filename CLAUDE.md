...
| 部件組合 | **組合本身**：點合、兩條軸、幾多層、交叉點講到啲咩（會自動配組合圖） |
...
7. **組合字一定要有 `combo` front matter 同 1.7 專章。**
   一隻形聲字唔係「兩嚿嘢黐埋」——佢同時屬於兩個家族：
   橫軸（同形符換聲符）同縱軸（同聲符換形符）。**兩軸嘅長短差異本身就係資料**
   （例：「財」嘅「貝」帶出成堆字，「才」只帶出三個；「靜」兩邊都長，
   所以先至會四說並列）。呢啲嘢淨係拆開兩邊講係睇唔出嚟嘅。
   1.7 專章要有：組合方式表、兩條軸嘅《說文》原文表、交叉點嘅發現、
   部件喺其他字嘅位置同變形、換位異體。
   組合圖用**會摺行嘅 HTML**，唔用 SVG —— 五格橫排喺手機一定要碌左右。
